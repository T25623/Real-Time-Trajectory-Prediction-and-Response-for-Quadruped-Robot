...
def open_rpi_camera(resolution={"size": (800, 600), "format": "RGB888"}, framerate={"FrameRate": 30}):
    try:
    ...
    try:
        picam2 = Picamera2()
        config = picam2.create_video_configuration(main=resolution, controls=framerate)
...
# -------------------------------------------------------------------
def init_input_source(input_src: str, batch_size: int, resolution: Optional[str], framerate: Optional[str]):
    """
    ...
    if src == "rpi":
        if not is_raspberry_pi():
            ...
            sys.exit(1)
        cap = open_rpi_camera(resolution, framerate)
        if cap is None:
            sys.exit(1)
