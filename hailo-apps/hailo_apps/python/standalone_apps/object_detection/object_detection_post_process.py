...
def extract_detections(image: np.ndarray, detections, config_data) -> dict:
    """
    Extract detections from HAILO NMS BY CLASS output.
    """
    ...
    for class_id, class_detections in enumerate(detections):
        ...
        # Skip if empty or scalar
        if class_detections is None:
            continue

        class_detections = np.array(class_detections)

        # If no detections for this class
        if class_detections.size == 0:
            continue

        # If single detection (1D), expand dims
        if class_detections.ndim == 1:
            class_detections = np.expand_dims(class_detections, axis=0)
